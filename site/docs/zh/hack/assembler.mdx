...
| --- | --- |
| `none` | 不添加行尾解释 |
| `summary` | ROM 地址和源码行号，例如 `ROM[0000] L4` |
| `full` | ROM 地址、源码文本和 Hack+ 展开 |

默认使用 `full`。`//%hack` 是机器可读的执行 metadata，因此所有级别都会保留。可以运行 `pixi run just hack asm multiply summary`，再与默认完整产物对照。
