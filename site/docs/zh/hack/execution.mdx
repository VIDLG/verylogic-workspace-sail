...
| `workflow.py` | 发现源码、选择程序、组织 `check/asm/run/test/clean` | ISA 语义和机器状态判断 |
...
`.hack` 和 driver 共用 `none / summary / full` 级别。`summary` 会给每个 ROM 分支标出地址与源码行，`full` 还会带上原始汇编和 Hack+ 展开，并解释执行阶段、断言来源、Hook 与最终输出。这些内容来自 `LoadedHack.word_comments`，说明注释确实穿过了磁盘文件边界。
