...
[中文](EXECUTION.zh-CN.md) · [Documentation index](../README.md) · [Hack ISA](ISA.md) · [Assembler internals](ASSEMBLER.md)

This guide follows [`executor.py`](../../isa/hack/tools/executor.py) and [`workflow.py`](../../isa/hack/tools/workflow.py) to explain how Hack machine code becomes a native executable through Sail's C backend, and how unit, ISA conformance, and assembly integration tests form one regression loop.

## Responsibility boundaries

| File | Responsible for | Not responsible for |
| --- | --- | --- |
| `hack.sail` | A/C instructions, ALU, registers, RAM, and PC transitions | ROM contents, a program-specific `main()`, test catalog |
| `executor.py` | Program driver generation, Sail/C compilation, execution, assertions | Program catalog and batch-test policy |
| `workflow.py` | Catalog selection and `check/asm/run/test/clean` orchestration | ISA semantics and judging machine state itself |
...
```mermaid
flowchart TD
    Just[just hack run multiply] --> Workflow[workflow.py selects catalog entry]
...
Steps 6 and 7 are the important design boundary. The executor does not pass an in-memory assembly object straight to driver generation; it forces data through the `.hack` artifact. See [Why `load_hack()` exists](ASSEMBLER.md#why-load_hack-exists).
...
function instruction_at(pc : program_counter) -> instruction = {
  match pc {
    0b000000000000000 => encdec(0b0000000000000110),
    0b000000000000001 => encdec(0b1110110000010000),
    // ...
...
[`test_hook_call_order_preserves_core_assertions`](../../isa/hack/tests/test_executor.py) fixes this contract by checking source order in the generated driver.
...
[`programs.toml`](../../isa/hack/programs.toml) contains only:

```toml
[[programs]]
name = "multiply"
source = "programs/multiply.asm"
description = "Repeated-addition multiplication: 6 times 7"
```

`catalog()` validates:

- `[isa]` and `[[programs]]` structure;
- nonempty string fields;
- unique names that are single safe path components;
- source paths that resolve inside the Hack package and exist.

Step limits, assertions, and hooks remain beside program semantics in `.asm`, not in the catalog.

### Action dispatch

| Action | Workflow behavior |
| --- | --- |
| `list` | Print ISA and catalog programs |
...
| `test` | Run package Pytest, then integration-run every catalog entry |
...
flowchart TD
    ...
    Test --> Catalog[for each programs.toml entry]
    Catalog --> Required[executor --require-assertions]
...
### Layer 3: catalog end-to-end tests

Workflow invokes every catalog program with:

```text
executor.py ... --require-assertions
```

An example without `.assert` fails immediately, preventing catalog entries that merely run without verifying anything.
...
8. **Batch workflow failure**: run one `just hack run NAME` before returning to catalog orchestration.
...
4. a duplicate name in `programs.toml`.
