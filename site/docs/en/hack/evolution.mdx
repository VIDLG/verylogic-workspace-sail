...
```text
0000000000000110 // ROM[0000] L4: SET R0, 6 // multiplicand [1/4] => @6
```

The generated driver receives the same mapping only after reloading the `.hack` file:
...
- **Tooling that still emits standard Hack words:** implement it in the existing module. New pseudoinstructions, traces, and diagnostics should preserve default behavior; enable experimental behavior through explicit syntax, a Hook, or an option.
- **A platform experiment:** prefer a separate Hook, driver option, or named recipe so the same standard ISA can run in different teaching environments. Consider a separate package only after the platform becomes a durable product.
...
Standard Hack currently has one small Sail file, so this workspace does not preemptively copy the RISC-V profile registry for variants that do not exist. If two or more durable Hack derivatives eventually coexist and demonstrably share stable implementation, extract common assembler or executor helpers from that real duplication. Do not build an abstraction framework first and make every student experiment conform to it.
...
- a hook that traces `PC`, the decoded instruction, and changed state;
...
| Execution observation | hook or executor/driver, deterministic tests, execution guide |
