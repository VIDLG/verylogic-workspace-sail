...
| `summary` | ROM address and source line, such as `ROM[0000] L4` |
| `full` | ROM address, source text, and Hack+ expansion |

`full` is the default. `//%hack` records are machine-readable execution metadata, so they are written at every level. Try `pixi run just hack asm multiply summary` and compare the result with the default full artifact.
