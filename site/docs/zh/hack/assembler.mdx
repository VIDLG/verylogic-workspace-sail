...
本文沿着 [`isa/hack/tools/assembler.py`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/tools/assembler.py) 与 [`artifact.py`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/tools/artifact.py)，解释源码怎样变成 profile-specific Hack 机器码，再跨越严格的持久化 artifact 边界。命令默认生成 16 位 `hack16`，`--profile hack32` 选择 32 位机器字。共享 directive 与 manifest 规则见[公共教学契约](/zh/reference/teaching-contract)。`assembler.py` 保持为纯库，`assembler_cli.py` 是处理参数与发布的薄 CLI 边界。
