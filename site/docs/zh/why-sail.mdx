...
共享的 [`model/core.sail`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/model/core.sail) 模型先保存旧状态，再明确写回：
...
上游 Sail 项目的 RISC-V 示例先定义指令形式，再把它与 32 位机器字连接：
...
这个上游示例展示了 Sail 的双向 `mapping` 能力。本仓库会在合法编码是紧凑双射时使用 mapping，例如 Hack；若还存在非法前缀，则再用显式 decode `match` 包装。RV32I 这类更复杂的模型则使用分离的 decode/encode 函数。
