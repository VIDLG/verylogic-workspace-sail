...
This guide follows [`isa/hack/tools/assembler.py`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/tools/assembler.py) and [`artifact.py`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/tools/artifact.py). It explains how source becomes 16-bit Hack machine code, then crosses a strict persisted-artifact boundary. Shared directive and manifest rules are defined by the [common teaching contract](/reference/teaching-contract). `assembler.py` remains a pure library; `assembler_cli.py` is the thin CLI boundary that handles arguments and publication.
