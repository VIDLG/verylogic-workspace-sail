...
| --- | --- | --- |
| `model/core.sail`, profile entry files/projects | Shared instruction/exception, ALU/state/fetch/decode/encode/execute/step, plus profile widths, legality, and mapping clauses | Loading a particular program image, a program-specific `main()`, test program discovery |
...
5. Create a temporary staging subdirectory beside the final artifact files
...
| `multiply.driver.sail_project` | `write_driver_project()` | Connects the generated driver to the selected fixed profile project |
...
- semantics from the selected profile project.
...
| `isa`, `profile`, `isa-metadata` | `load_hack()` requires `isa=hack` and `profile=hack16` or `hack32`, checks constants against the selected model, and rejects tampering or mismatches. `standard` is never a valid profile. `loaded_rom_words` comes from the reloaded word count; architectural widths and capacities belong to the selected project. |
...
fetch_hack(PC) -> decode_hack(word) -> execute(decoded instruction) -> unit
                              illegal word -> throw HackIllegalInstruction(word)
```
...
| `check` | Check both profile projects; `--profile hack32` narrows to one |
| `assemble NAME` | Assemble the selected profile (default `hack16`) into `.build/<profile>/asm/NAME/NAME.hack` atomically |
| `run NAME` | Assemble, generate, compile, and run the selected profile |
| `test` | Run the Python/tooling, direct Sail, and discovered-program matrices for both profiles |
| `clean` | Recreate the profile-aware `.build/` structure |
