...
def write_driver(program: LoadedHack, max_steps: int | None, path: Path, binary: Path) -> None:
    words = program.words
    metadata = program.metadata
    cases = "\n".join(
        f"    0b{address:015b} => encdec(0b{word:016b})," for address, word in enumerate(words)
    )
    completion_cases = "\n".join(
    ...
    path.write_text(
        f"""// Generated by executor.py from {binary.name}; do not edit.
...
def run(program: Path, output: Path, max_steps: int | None = None, require_assertions: bool = False) -> None:
    ...
    write_hack(assembly, machine_code)
    ...
    write_driver(reloaded, effective_max_steps, driver, machine_code)
...
def main() -> int:
    ...
    try:
        run(Path(args.program), Path(args.output), args.max_steps, args.require_assertions)
