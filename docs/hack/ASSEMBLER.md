...
[中文](ASSEMBLER.zh-CN.md) · [Documentation index](../README.md) · [Hack ISA](ISA.md) · [Execution and tests](EXECUTION.md)

This guide follows [`isa/hack/tools/assembler.py`](../../isa/hack/tools/assembler.py) and explains how a small but complete assembler turns `.asm` source into 16-bit Hack machine code. It is not an API listing; it is a guided study of compiler phases and why they occur in this order.
...
The output `AssemblyResult` has two channels:

1. `records`: real `MachineWord` values that enter ROM;
2. `metadata`: HALT addresses, assertions, a step limit, and a hook path.

This split prevents `.assert` and similar test facilities from pretending to be Hack instructions while still allowing them to travel with the `.hack` artifact to the executor.
...
- duplicate `.hook` or `.max_steps` directives;
...
```text
ROM[0000] L1: SET R0, 6 => @6
ROM[0001] L1: SET R0, 6 => D=A
```
...
Dot directives come first so an invalid `.assert` gets an assertion-specific error rather than an ambiguous “bad C instruction.” Handling `@` separately similarly distinguishes an invalid operand from a non-A instruction.
...
Each lowered instruction stores its standard form in `expanded`, while `source` still points at the original `JEQ` line.
...
See the [ISA guide](ISA.md#how-hack-lowers-to-real-instructions) for the complete Hack+ lowering table.
...
```python
MachineWord(value, source, expanded)
```

Machine code, original source, and lowering provenance remain connected.
...
```text
0000000000000110 // ROM[0000] L1: SET R0, 6 => @6
1110110000010000 // ROM[0001] L1: SET R0, 6 => D=A
```
...
[`test_assembler.py`](../../isa/hack/tests/test_assembler.py) fixes contracts at every phase rather than checking only a few example programs:
...
Continue with [Execution and test workflow](EXECUTION.md) to follow this `.hack` artifact into a running program.
