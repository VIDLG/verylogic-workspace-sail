...
4. 检查断言要求与 ROM 大小
5. 创建临时 sibling staging directory
...
完整 `run` 在临时 sibling directory 中创建、编译并执行这些文件，只有 execution/assertions 成功才发布整个 closure。Backup-and-replace 发布在安装失败时恢复先前目标；staged build 或 run 失败会保留先前已发布 closure。
...
| `isa`、`profile`、`isa-metadata` | `load_hack()` 检查 artifact 的 `hack/standard` identity 与常量是否精确匹配当前所选模型，并拒绝篡改或不一致；它们不会生成 driver 常量。`loaded_rom_words` 来自实际重载的机器字数量，架构位宽和容量则由 `hack.sail` 定义。 |
...
生成的 `main()` 先加载原始机器字，在 completion 与 watchdog 控制下执行经过检查的模型步骤，最后求值源码断言：
...
Driver 负责 completion 检查、watchdog、源码断言和最终输出。取指、解码和执行仍是模型自有的一个 `hack_step()`；生成的 `main()` 只把类型化结果转换为 driver 策略。
...
Sail 对两个文件整体做类型检查，并生成 `<output>.c` 与 `<output>.h`。
...
| `assemble NAME` | 调用 assembler CLI，原子写 `.build/NAME.hack` |
...
`tests/isa_conformance.sail` 直接检查 `alu()`、`should_jump()` 和 `execute()`，覆盖 ALU 运算、跳转真值表、destination 掩码、旧 `A` 语义和 PC 回绕。这一层独立验证 Sail ISA 函数，不依赖 generated-driver 集成路径。
