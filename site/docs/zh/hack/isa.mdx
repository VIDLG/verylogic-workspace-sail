...
本文只讲 **Hack ISA 本身**：软件能够看到什么状态、机器指令怎样编码，以及一条指令怎样改变状态。运行命令、断言和 `.hack` 元数据等工具规则见 [Hack 包参考手册](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/README.zh-CN.md)。
...
| ROM | 32768 × 16 位 | 程序机器字 | `register ROM : vector(32768, word)`；由 driver 加载、模型取指 |
...
这正是 [`hack.sail`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/hack.sail) 中先执行 `let old_a = A` 的原因。
...
[`hack.sail`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/hack.sail) 的结构几乎与本文一一对应：

| Sail 定义 | ISA 概念 |
| --- | --- |
| `type word = bits(16)` | 16 位机器字和数据字 |
| `input_selector` / `alu_control` / `destination_control` / `jump_control` | C 指令各字段的命名位向量类型 |
| `union instruction` | A/C 两种解码结果 |
| `mapping encdec` | 合法 A/C 编码的紧凑双向关系 |
| `union hack_decode_result` | 显式的成功或非法解码结果 |
| `decode_hack` / `encode_hack` | 检查合法性的 decode 包装与基于 mapping 的 encode 包装 |
| `register ROM/A/D/PC/RAM` | 指令存储与架构状态 |
| `fetch_hack` | 按程序计数器从 ROM 读取一个原始机器字 |
| `alu` | `comp` 字段语义 |
| `should_jump` | `jump` 真值表 |
| `execute` | 已解码指令的状态转换 |
| `hack_step` / `hack_execution_outcome` | 模型自有的取指 → 解码 → 执行组合及类型化结果 |

C 指令字段使用命名位向量类型，而不是匿名的 `bit` / `bits(n)` 位置。这使 `CInstruction`、`alu` 和 `should_jump` 的接口能够直接表达字段角色，同时不改变原始编码域。

`encdec` 只写一次合法 A/C 位布局，并可双向使用。`decode_hack` 先显式匹配第 15 位，再匹配 C 指令的 `11` 前缀，确认合法后才调用 mapping。`100`、`101`、`110` 前缀返回 `DecodeIllegal(encoded)` 并保留原始机器字；通配分支不会静默构造合法指令。`encode_hack` 暴露 mapping 的反方向。`execute` 只消费已经解码的 `instruction`，因此不会直接调用 `encdec`。

标准汇编器只生成上面表格中的规范 `comp` 编码。Sail 解码器把 `a` 与六位 `comp` 分开建模；对于不读取 `y` 的运算，非规范的 `a=1` 位型在电路和本模型中会表现为同义编码，但项目汇编器不会生成它们。

## 模型边界

理解测试结果时要区分完整 Hack 平台与本可执行模型：

- ROM 在 `hack.sail` 中声明；生成的 driver 初始化原始机器字，`fetch_hack` 和 `hack_step` 负责取指与分派；
...
2. 打开 [`hack.sail`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/hack.sail)，依次阅读 `encdec → decode_hack → encode_hack → fetch_hack → alu → should_jump → execute → hack_step`；
...
4. 执行 `pixi run just hack assemble basic_alu`，查看 `isa/hack/.build/basic_alu.hack`；
5. 阅读 [`tests/isa_conformance.sail`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/tests/isa_conformance.sail)，观察 ISA 规则如何直接变成测试。
