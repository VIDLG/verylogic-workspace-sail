...
| `summary` | ROM address and source line; Hack+ words also show `[i/n] source => canonical` |
| `full` | Summary information plus the complete original source text |

`summary` is the default. It keeps pseudoinstruction lowering visible while leaving ordinary A/C instructions compact. `//%hack` records are machine-readable execution metadata, so they are written at every level. Run `pixi run just hack assemble multiply`, then request `full` explicitly when you need the complete source text and driver explanations.
