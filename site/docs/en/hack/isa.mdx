...
| ROM | 32768 × 16 bits | Program machine words | `register ROM : vector(32768, word)`; loaded by the driver, fetched by the model |
...
That is why [`hack.sail`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/hack.sail) begins the C-instruction path with `let old_a = A`.
...
The structure of [`hack.sail`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/hack.sail) follows this document closely:

| Sail definition | ISA concept |
| --- | --- |
| `type word = bits(16)` | 16-bit machine and data words |
| `input_selector` / `alu_control` / `destination_control` / `jump_control` | Named bit-vector types for C-instruction fields |
| `union instruction` | A/C decoded forms |
| `mapping encdec` | Compact bidirectional relation for legal A/C encodings |
| `union hack_decode_result` | Explicit decoded or illegal result |
| `decode_hack` / `encode_hack` | Legality-checking decode wrapper and mapping-based encode wrapper |
| `register ROM/A/D/PC/RAM` | Instruction storage and architectural state |
| `fetch_hack` | Read one raw word from ROM at a program counter |
| `alu` | `comp` semantics |
| `should_jump` | `jump` truth table |
| `execute` | State transition for an already-decoded instruction |
| `hack_step` / `hack_execution_outcome` | Model-owned fetch → decode → execute composition and typed result |

The C-instruction fields use named bit-vector types rather than anonymous `bit`/`bits(n)` positions. This documents their roles throughout `CInstruction`, `alu`, and `should_jump` without changing the raw encoding domain.

`encdec` states the legal A/C bit layout once and works in both directions. `decode_hack` explicitly matches bit 15 and then the C-instruction `11` prefix before calling that mapping. Prefixes `100`, `101`, and `110` return `DecodeIllegal(encoded)` and preserve the raw word; no wildcard silently constructs a legal instruction. `encode_hack` exposes the reverse mapping direction. `execute` consumes an already-decoded `instruction`, so it does not call `encdec` directly.

The standard assembler emits only the canonical `comp` encodings in the tables above. Sail models `a` separately from the six `comp` bits; for operations that do not read `y`, noncanonical encodings with `a=1` behave as aliases in the circuit and this model, but this project's assembler never emits them.

## Model boundaries

Keep the complete Hack platform separate from this executable model when interpreting results:

- ROM is declared in `hack.sail`; the generated driver initializes its raw words, while `fetch_hack` and `hack_step` own instruction fetch and dispatch.
...
2. Open [`hack.sail`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/hack.sail) and follow `encdec → decode_hack → encode_hack → fetch_hack → alu → should_jump → execute → hack_step`.
...
4. Run `pixi run just hack assemble basic_alu` and inspect `isa/hack/.build/basic_alu.hack`.
5. Read [`tests/isa_conformance.sail`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/tests/isa_conformance.sail) to see ISA rules turned directly into tests.
