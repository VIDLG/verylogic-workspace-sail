...
The default `summary` level keeps ROM addresses and source lines compact while marking every Hack+ expansion as `[i/n] source => canonical`. Explicit `full` adds complete source text and more driver-stage and assertion explanations. Use `none` when a bare artifact is more useful than teaching annotations.
