...
[English](EXECUTION.md) · [文档中心](../README.zh-CN.md) · [Hack ISA](ISA.zh-CN.md) · [汇编器原理](ASSEMBLER.zh-CN.md)

本文沿着 [`executor.py`](../../isa/hack/tools/executor.py) 和 [`workflow.py`](../../isa/hack/tools/workflow.py) 的调用链，解释 Hack 机器码怎样通过 Sail C 后端变成宿主机可执行程序，以及单元测试、ISA 一致性测试和汇编程序集成测试怎样组成闭环。

## 三个文件的职责边界

| 文件 | 负责什么 | 不负责什么 |
| --- | --- | --- |
| `hack.sail` | A/C 指令、ALU、寄存器、RAM、PC 状态转换 | ROM 内容、特定程序的 `main()`、测试清单 |
| `executor.py` | 生成程序专属 driver、调用 Sail/C 编译、运行并检查断言 | 程序目录、批量测试策略 |
| `workflow.py` | 读取 catalog、选择程序、组织 `check/asm/run/test/clean` | ISA 语义和机器状态判断 |
...
最关键的设计点是第 6、7 步。executor 不把汇编阶段的内存对象直接传到 driver，而是强制经过 `.hack` 文件边界。详见 [汇编器原理：为什么要重新加载](ASSEMBLER.zh-CN.md#为什么还要-load_hack)。
...
[`test_hook_call_order_preserves_core_assertions`](../../isa/hack/tests/test_executor.py) 通过检查生成源码中的位置顺序固定了这份契约。
...
[`programs.toml`](../../isa/hack/programs.toml) 只保存：
...
- 字段是非空字符串；
- 名称不重复，且只是单个安全路径组件；
- 源文件解析后仍位于 Hack 包内并真实存在。

步数、断言和 Hook 不放在 catalog；它们与程序语义一起留在 `.asm` 源码中。

### action 分派

| action | workflow 行为 |
| --- | --- |
| `list` | 打印 ISA 与所有 catalog 程序 |
...
| `test` | 先运行包内 Pytest，再逐个集成运行 catalog 程序 |
...
workflow 对每个 catalog entry 调用：

```text
executor.py ... --require-assertions
```

没有 `.assert` 的示例会直接失败，避免 catalog 中出现“只运行、什么也没验证”的假测试。
...
2. **编码错误**：运行 `just hack asm NAME`，检查 `.hack` 的源码映射；
...
8. **批量工作流错误**：先直接运行单个 `just hack run NAME`，再回到 catalog。
