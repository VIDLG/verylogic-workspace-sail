...
- **仍生成标准 Hack 机器码的工具改进**：可以在原模块中实现，例如新伪指令、trace 或更好的诊断。默认行为应保持兼容；实验性行为通过显式语法、driver 选项或命名 recipe 启用。
- **平台实验**：优先使用 driver 选项或命名 recipe，使同一标准 ISA 可以运行在不同教学环境中。只有平台成为长期稳定产品时，才考虑独立包。
...
标准 Hack 有意由一个小型 Sail 文件表示；工作区不为不存在的变体增加 profile registry。若两个以上需要长期共存的 Hack 派生机器确实共享大量稳定实现，再从真实重复中提取公共 assembler 或 executor helper；不要先做抽象框架，再要求每个学生实验适配它。
...
| 执行观察能力 | executor/driver、确定性测试、执行文档 |
