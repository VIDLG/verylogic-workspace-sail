...
Named bit-vector types make each C-instruction field's role visible without changing its raw encoding. The compact `encdec` mapping states the legal A/C encoding in both directions. `decode_hack` adds an explicit legality `match`: valid prefixes use `encdec`, while `100/101/110` preserve the raw word in `DecodeIllegal`. `execute` receives only the decoded `instruction`; it does not perform encoding or decoding.
...
1. Read `encdec`, `decode_hack`, and `encode_hack` in `hack.sail`.
