...
Before changing or adding an ISA model, follow [`.design/sail/MODELING.md`](.design/sail/MODELING.md).
...
- Unknown or reserved encodings must throw a dedicated typed Sail exception, produce an explicit illegal/reserved result, or be a documented non-match. Prefer an exception when the only valid policy is to abort the current step; prefer a result when callers need recovery or architectural dispatch. A wildcard must never silently construct a legal instruction.
