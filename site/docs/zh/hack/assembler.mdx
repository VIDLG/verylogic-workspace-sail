...
默认使用 `summary`：普通 A/C 源码和伪指令展开都会显示，行内汇编注释放在最右侧。`//%hack` 是机器可读的执行 metadata，因此所有级别都会保留。可以运行 `pixi run just hack assemble multiply`，也可以用 `pixi run just hack assemble multiply summary --hook hooks/trace.sail --max-steps 10000` 覆盖运行 metadata。CLI 覆盖先经 `apply_runtime_overrides()` 应用，再调用 `write_hack()`，因此 artifact 会记录最终有效值。
