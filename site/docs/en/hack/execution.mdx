...
The same `none / summary / full` level used for `.hack` controls driver comments. At the default `summary`, each ROM arm carries its address and source line, and Hack+ words also carry `[i/n] source => canonical`. `full` adds complete original source text and expands the explanations for assertion sources, hooks, and final output. These comments come from `LoadedHack.word_comments`, proving they survived the file boundary.
...
2. **Encoding error**: run `just hack assemble NAME` and inspect `.hack` source mappings.
