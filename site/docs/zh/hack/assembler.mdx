...
[← Hack ISA](/zh/hack/isa) · [公共教学契约](/zh/reference/teaching-contract) · [English](/hack/assembler) · [执行器与测试 →](/zh/hack/execution)

本文沿着 [`isa/hack/tools/assembler.py`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/tools/assembler.py) 与 [`artifact.py`](https://github.com/VIDLG/verylogic-workspace-sail/blob/main/isa/hack/tools/artifact.py)，解释源码怎样变成 16 位 Hack 机器码，再跨越严格的持久化 artifact 边界。共享 directive 与 manifest 规则见[公共教学契约](/zh/reference/teaching-contract)。`assembler.py` 保持为纯库，`assembler_cli.py` 是处理参数与发布的薄 CLI 边界。
...
| `DescriptionDirective` | `.description` | 否，供 discovery 与 manifest metadata 使用 |
...
`write_hack()` 委托公共 renderer。`summary` 和 `full` 在文件开头写入一个连续的多行缩进 canonical S-expression block，每一行都带 `//% ` 前缀；`none` 把同一 form 写成一条带前缀的紧凑行。Schema 是 `verylogic.annotated-image`；公共 envelope 包含 runtime `max_steps`、assertions、completion 与 ISA metadata。Hack 直接汇编省略空 frontend `provenance`。Hack-specific validation 固定 `hack/standard`、source kind `asm`、16 位 word、memory dimensions 与 lowered-self-loop completion。Runtime value 同时记录 `cli`、`source` 或 `default` origin。
...
| 级别 | 机器字注释 |
| --- | --- |
| `none` | 文件开头一条紧凑 manifest 行后直接写机器字；无 human preamble 或行尾解释 |
| `summary` | 每个机器字都显示 ROM/源码位置和规范化汇编；Hack+ 增加 `[i/n] 源伪指令 => 正式指令`；行内注释放在最右侧 |
| `full` | summary 信息加精确的完整原始源码文本 |
...
- 每个机器字恰好 16 位且只含 `0/1`，注释是否存在符合 manifest level；
- completion address 真实指向 `@address; 0;JMP` 自循环；
- ROM 不超过 32768 个机器字。

`load_hack()` 只接受严格带注释格式，没有文件开头 manifest block 的文件会被拒绝。未来若增加原始 nand2tetris `.hack` 互操作，必须使用独立显式 frontend，因为 raw image 无法如实提供 description、assertions、watchdog 来源或 completion metadata。
