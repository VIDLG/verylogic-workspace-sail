...
[English](ASSEMBLER.md) · [文档中心](../README.zh-CN.md) · [Hack ISA](ISA.zh-CN.md) · [执行与测试](EXECUTION.zh-CN.md)

本文以 [`isa/hack/tools/assembler.py`](../../isa/hack/tools/assembler.py) 为主线，解释一个小型但完整的汇编器怎样把 `.asm` 源码变成 16 位 Hack 机器码。它不是 Python API 逐函数抄录，而是一次“编译器阶段为什么要这样排”的导读。
...
输出 `AssemblyResult` 也有两条通道：

1. `records`：真正进入 ROM 的 `MachineWord`；
2. `metadata`：HALT 地址、断言、步数限制和 Hook 路径。

这种分离保证 `.assert` 等测试能力不会伪装成 Hack 指令，同时又能随 `.hack` 产物流转到 executor。
...
| `HookDirective` | `.hook` | 否，进入 metadata |

解析优先级是有意安排的：

1. 去掉 `//` 后的注释和首尾空白；
2. 识别 `.assert`、`.max_steps`、`.hook`；
3. 识别标签；
4. 识别以 `@` 开头的 A 指令；
5. 根据首个单词识别 Hack+；
6. 剩余内容按 C 指令解析。

点指令必须先于普通指令，否则 `.assert` 可能得到含糊的“非法 C 指令”错误。以 `@` 开头的内容单独处理，则能把“非法操作数”和“根本不是 A 指令”区分开。
...
每条展开指令的 `expanded` 字段记录自己的标准写法，但 `source` 仍指向原始 `JEQ` 行。
...
完整 Hack+ 展开表见 [ISA 文档](ISA.zh-CN.md#hack-如何降级为正式指令)。
...
[`test_assembler.py`](../../isa/hack/tests/test_assembler.py) 不是只测几个示例程序，它固定了每个阶段的契约：
...
下一步阅读 [执行器与测试工作流](EXECUTION.zh-CN.md)，继续追踪这个 `.hack` 文件怎样变成真正运行的程序。
